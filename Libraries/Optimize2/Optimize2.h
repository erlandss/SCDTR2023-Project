...
class Optimizer{
    ...
    explicit Optimizer(float _rho,float _o,float l,float* _K,float* _c, int ind):
    rho{_rho},o {_o},L {l}, K {_K}, c {_c}, index {ind}
    {
    ...
    void solveQP(){
        ...
        for(int i=1;i<6;i++){
            ...
            if(costFunction(d_opt)<costFunction(d) && (dot_product(sol0Temp,d_opt,N)<=(o-L) && dot_product(sol1Temp,d_opt,N)<=0 && dot_product(e,d_opt,N)<=100) ){
                ...
                memcpy(&(d[0]),&(d_opt[0]),sizeof(float));
                memcpy(&(d[1]),&(d_opt[1]),sizeof(float));
                memcpy(&(d[2]),&(d_opt[2]),sizeof(float));
